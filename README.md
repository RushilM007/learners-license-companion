# Learner's License Companion

## About 
Preparing for the Indian learner's license exam with just the official practice PDF isnt the most amazing experience. This app turns the practice questions into an interactive questionbank where you can practice questions based on category and can bookmark difficult questions. It also has a mock exam that you can take in the official format, and a chatbot that can answer your conceptual queries. 

## Tech Stack 
- Firebase for authentication and storing user data 
- Supabase for storing vector embeddings 
- HTML, CSS, Javascript, React.JS, Node.JS, Express.JS 
- Claude API for generating feedback for the users' mock exam performances, OpenAI API for the RAG query chatbot 

## How to Use

### Clone the Repo
-git clone https://github.com/RushilM007/learners-license-companion
-cd learners-license-companion

### Install Dependencies
npm install

### Set up Environment Variables
Go to the .env.example file. Enter your claude API key and your OpenAI API key. 

### Run the App
-npm run server
-npm run dev



